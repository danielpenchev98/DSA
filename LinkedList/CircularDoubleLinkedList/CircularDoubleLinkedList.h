...
{
	Node<T, Y>* newElement = new Node<T, Y>;
	newElement->data = A.data;
	newElement->key = A.key;
	last = newElement;
...
inline void CircularDoubleLinkedList<T, Y>::CopyList(const CircularDoubleLinkedList<T, Y>&A)
{
	...
	newElement = new Node<T, Y>;
	newElement->data = toCopy->data;
	newElement->key = toCopy->key;
...
inline void fillList(Node<T, Y>*& toFill, Node<T, Y>* source1, Node<T, Y>* source2)
{
	...
	while (source1 != nullptr&&source2 != nullptr)
	{
		...
		{
			temp->data = source1->data;
			temp->key = source1->key;
			source1 = source1->next;
		}
		else
		{
			temp->data = source2->data;
			temp->key = source2->key;
			source2 = source2->next;
		}
		if (newStart == nullptr)
		{
...
template<typename T,typename Y>
inline CircularDoubleLinkedList<T, Y>::Iterator& CircularDoubleLinkedList<T, Y>::Iterator::operator=(const Iterator&A)
{
	if (this != &A)
	{
		curr = A.curr;
