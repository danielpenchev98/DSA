#pragma once
enum Color{doubleblack =0,black,red};
//could add sentinel node which will be excellent but dont wanna waste time to rewrite 700 lines of code
template<typename T>
struct Node
{
	Node* left;
...
public:
	...
	virtual void InsertNode(const T&, const int);
	virtual void InsertNode(const Node<T>&);
	...
	Node<T>* GetMinNode();
	const Node<T>* const SubTreeMin(const Node<T>* const) const ;
	Node<T>* SubTreeMin(const Node<T>* const);
	T& maxData();
...
inline void BinarySearchTree2<T>::CopyTree(const BinarySearchTree2<T>&A)
{
	...
	else
	{
		...
		catch (const std::bad_alloc& maybe)
		{
...
inline void BinarySearchTree2<T>::Pre(const Node<T>* const curr) const
{
	...
	std::cout << curr->data << " with color: " << (curr->col==red?"red" : "black") << std::endl;
	Pre(curr->left);
	Pre(curr->right);
}
template<typename T>
...
template<typename T>
Node<T>* BinarySearchTree2<T>::SubTreeMin(const Node<T>*const A)
{
	return const_cast<Node<T>*>(static_cast<const BinarySearchTree2<T>&>(*this).SubTreeMin(A));
}
//////////////////////////////////////
