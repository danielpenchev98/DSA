...
private:
	...
	void caseA(Node<T>*& , Node<T>*& );

	        //-has either left or right child

	void caseB(Node<T>*& , Node<T>*& );

	        //-has both left and right children

	void caseC(Node<T>*& , Node<T>*& );
...
template<typename T>
inline void BinarySearchTree2<T>::FindNodeAndParent(const int KEY, Node<T>*&loc, Node<T>*&par)const
{
	if (root == nullptr)
	{
		loc = nullptr;
...
template<typename T>
inline void BinarySearchTree2<T>::caseA(Node<T>*& par, Node<T>*& loc)
{
	if (par == nullptr)
	{
		root = nullptr;
...
template<typename T>
inline void BinarySearchTree2<T>::caseB(Node<T>*& par, Node<T>*& loc)
{
...
template<typename T>
inline void BinarySearchTree2<T>::caseC(Node<T>*& par, Node<T>*& loc)
{
	Node<T>* ptr = nullptr;
	Node<T>* ptrsave = nullptr;
	Node<T>* suc = nullptr;
	Node<T>* parsuc = nullptr;
	ptrsave = loc;
	ptr = loc->right;
...
inline void BinarySearchTree2<T>::DeleteNode(const int KEY)
{
	...
	FindNodeAndParent(KEY, location, parent);
	//std::cout << parent->key << "\n" << location->data << std::endl;
	if (location == nullptr)
	{
		return;
	}
...
inline Node<T>* BinarySearchTree2<T>::RightRotation(Node<T>* node)
{
	...
	if (Subroot != nullptr)
	{
		Subroot->left == node ? Subroot->left = node : Subroot->right = node;
	}
	else
	{
		root = B;
...
inline unsigned int BinarySearchTree2<T>::GetHeight(const Node<T>* const curr, const T& DATA,const int KEY, const unsigned int counter) const
{
	...
	int temp1 = GetHeight(curr->left,DATA,KEY, counter + 1);
	int temp2 = GetHeight(curr->right,DATA,KEY, counter + 1);
	if (temp1 == 0 && temp2 == 0)
	{
		return 0;
	}
